...
class EventRepository {
  ...
  Future<List<Event>> fetchEventsToday() async {
    if (!SupabaseGate.isEnabled) return [];
    final now = DateTime.now();
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('⚠️ EventRepository: fetchEventsToday failed: $e');
      }
      return [];
    }
  }

  Future<List<Event>> fetchHighlightEventsThisYear() async {
    if (!SupabaseGate.isEnabled) return [];
    final now = DateTime.now();
    final startOfYear = DateTime(now.year, 1, 1);
    final startOfNextYear = DateTime(now.year + 1, 1, 1);
    final startUtc = startOfYear.toUtc().toIso8601String();
    final endUtc = startOfNextYear.toUtc().toIso8601String();
    try {
      final response = await SupabaseGate.client
          .from('events')
          .select()
      ...
      if (kDebugMode) {
        debugPrint('⚠️ EventRepository: fetchHighlightEventsThisYear failed: $e');
      }
      return [];
    }
  }
